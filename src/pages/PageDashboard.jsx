      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 14, color: L.t1 }}>Metas do Mês</div>
        ...
        {!meta ? (
          ...
          <>
            <ProgressBar label="Meta de Consultas" actual={consultasReais} total={meta.meta_consultas || 0} color={L.teal} />
            <ProgressBar label="Meta de Receita" actual={receitaReal} total={Number(meta.meta_receita) || 0} color={L.green} formatFn={fmt} />
          </>
        )}
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <label style={{ display: 'block', marginBottom: 16 }}>
              ...
              <input type="number" min="0" value={form.meta_consultas}
                onChange={e => setForm(f => ({ ...f, meta_consultas: e.target.value }))}
                placeholder="Ex: 150"
                style={{ width: '100%', height: 40, padding: '0 12px', borderRadius: 9, border: `1px solid ${L.line}`, background: L.surface, fontSize: 14, outline: 'none' }}
              />
            </label>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={saveMeta} disabled={saving} className="btn-primary">{saving ? 'Salvando…' : 'Salvar Metas'}</button>
            </div>
          </div>
        </div>
      )}
...
function Aniversariantes({ clinicaId }) {
  ...
  function parseNasc(dt) { return new Date(dt + 'T12:00:00') }
  ...
  const semana = pacientes
    ...
    .sort((a, b) => {
      const dA = parseNasc(a.data_nascimento), dB = parseNasc(b.data_nascimento)
  ...
  return (
    <div style={{
      background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14,
      padding: 20, marginBottom: 20, borderLeft: `4px solid ${L.yellow}`,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {['semana', 'mes'].map(t => (
            ...
            }}>{t === 'semana' ? 'Esta Semana' : 'Este Mês'}</button>
          ))}
        </div>
      </div>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontWeight: 600, color: L.t1, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.nome}</span>
                      {hoje && <span style={{ padding: '1px 7px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: L.yellow, color: '#fff', flexShrink: 0 }}>🎉 Hoje!</span>}
                    </div>
                    <div style={{ fontSize: 11, color: L.t4, fontFamily: "'JetBrains Mono', monospace", marginTop: 1 }}>
                      {fmtNasc(p.data_nascimento)} · {age(p.data_nascimento)} anos
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 20, color: L.t1 }}>Painel C4HUB 👋</div>
        <div style={{ fontSize: 14, color: L.t3, marginTop: 2 }}>{new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</div>
      </div>
      ...
      <div style={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14, padding: 20 }}>
        <div style={{ fontWeight: 600, fontSize: 14, color: L.t1, marginBottom: 16 }}>Clientes cadastrados ({clientes.length})</div>
        {clientes.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0', color: L.t4 }}>Nenhum cliente cadastrado.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${L.line}` }}>
                {['Nome', 'Tipo', 'Plano', 'Localização', 'Status'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 11, color: L.t4, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {clientes.map((c, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '11px 12px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 600, background: c.tipo === 'hospital' ? L.purpleBg : L.blueBg, color: c.tipo === 'hospital' ? L.purple : L.blue }}>{c.tipo === 'hospital' ? 'Hospital' : 'Clínica'}</span>
                  </td>
                  <td style={{ padding: '11px 12px', fontSize: 12, color: L.t3, textTransform: 'capitalize' }}>{c.plano || 'básico'}</td>
                  <td style={{ padding: '11px 12px', fontSize: 13, color: L.t2 }}>{[c.cidade, c.estado].filter(Boolean).join(' / ') || '—'}</td>
                  <td style={{ padding: '11px 12px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 600, background: c.ativo ? L.greenBg : L.redBg, color: c.ativo ? L.green : L.red }}>{c.ativo ? 'Ativo' : 'Inativo'}</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
function DashboardClinica({ profile }) {
  ...
  useEffect(() => { if (!clinicaId) { setLoading(false); return }; load() }, [clinicaId])
  ...
  async function load() {
    ...
    try {
      ...
      const [agHoje, totalPac, receitaMes, novosMes, consultasMes] = await Promise.all([
        supabase.from('agendamentos').select('*, pacientes(nome), medicos(nome)').eq('clinica_id', clinicaId).gte('data_hora', `${hoje}T00:00:00`).lte('data_hora', `${hoje}T23:59:59`).order('data_hora'),
        supabase.from('pacientes').select('id', { count: 'exact', head: true }).eq('clinica_id', clinicaId).eq('ativo', true),
        supabase.from('financeiro_lancamentos').select('valor').eq('clinica_id', clinicaId).eq('tipo', 'receita').eq('status', 'pago').gte('data_pagamento', inicioMes),
        supabase.from('pacientes').select('id', { count: 'exact', head: true }).eq('clinica_id', clinicaId).gte('criado_em', inicioMes),
        supabase.from('agendamentos').select('id', { count: 'exact', head: true }).eq('clinica_id', clinicaId).gte('data_hora', inicioMes),
      ])
      const receita = (receitaMes.data || []).reduce((s, r) => s + Number(r.valor), 0)
      setAgendamentosHoje(agHoje.data || [])
      setKpis({ hoje: agHoje.data?.length || 0, pacientes: totalPac.count || 0, receita, novosMes: novosMes.count || 0, consultasMes: consultasMes.count || 0 })
      const dias = []
      for (let i = 6; i >= 0; i--) {
        ...
        const [{ count: qtd }, { data: fin }] = await Promise.all([
          supabase.from('agendamentos').select('*', { count: 'exact', head: true }).eq('clinica_id', clinicaId).gte('data_hora', `${ds}T00:00:00`).lte('data_hora', `${ds}T23:59:59`),
          supabase.from('financeiro_lancamentos').select('valor').eq('clinica_id', clinicaId).eq('tipo', 'receita').eq('status', 'pago').gte('data_pagamento', ds).lte('data_pagamento', ds),
        ])
        dias.push({ dia: label, consultas: qtd || 0, receita: (fin || []).reduce((s, r) => s + Number(r.valor), 0) })
      }
      setChartData(dias)
    } catch (e) { console.warn('Erro ao carregar dashboard:', e) } finally { setLoading(false) }
  }
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 20, color: L.t1 }}>Bom dia! 👋</div>
        <div style={{ fontSize: 14, color: L.t3, marginTop: 2 }}>{new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }} className="grid-cols-4">
      ...
      <MetasMensais clinicaId={clinicaId} consultasReais={kpis.consultasMes} receitaReal={kpis.receita} />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div style={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14, padding: 20 }}>
          <div style={{ fontWeight: 600, fontSize: 14, color: L.t1, marginBottom: 16 }}>Consultas — últimos 7 dias</div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 8, fontSize: 12 }} formatter={v => [v, 'consultas']} />
              <Area type="monotone" dataKey="consultas" stroke={L.teal} strokeWidth={2} fill="url(#tealGrad)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div style={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14, padding: 20 }}>
          <div style={{ fontWeight: 600, fontSize: 14, color: L.t1, marginBottom: 16 }}>Receita — últimos 7 dias</div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" stroke={L.line} vertical={false} />
              <XAxis dataKey="dia" tick={{ fontSize: 11, fill: L.t4 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: L.t4 }} axisLine={false} tickLine={false} tickFormatter={v => `R$${v}`} />
              <Tooltip contentStyle={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 8, fontSize: 12 }} formatter={v => [fmt(v), 'receita']} />
      ...
      <div style={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
        <div style={{ fontWeight: 600, fontSize: 14, color: L.t1, marginBottom: 16 }}>Agenda de hoje ({agendamentosHoje.length})</div>
        {agendamentosHoje.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0', color: L.t4, fontSize: 14 }}>Nenhum agendamento para hoje</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 1fr 1fr 100px', padding: '8px 12px', fontSize: 11, color: L.t4, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.3px', borderBottom: `1px solid ${L.line}` }}>
              <span>HORA</span><span>PACIENTE</span><span>MÉDICO</span><span>TIPO</span><span>STATUS</span>
            </div>
            {agendamentosHoje.map(ag => (
              <div key={ag.id} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 1fr 1fr 100px', padding: '10px 12px', fontSize: 13, borderRadius: 8, transition: 'background 0.12s' }}
                onMouseEnter={e => e.currentTarget.style.background = L.hover}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <span style={{ color: L.teal, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>{fmtHora(ag.data_hora)}</span>
...
function Spinner() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300 }}>
      <div style={{ width: 28, height: 28, border: `3px solid ${L.line}`, borderTop: `3px solid ${L.teal}`, borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    </div>
  )
}
