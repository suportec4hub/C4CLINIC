...
function AcoesRapidas({ nav }) {
  const acoes = [
    ...
    { id: 'agenda',     icon: '◷', label: 'Agendar',       cor: L.blue,   bg: L.blueBg },
    { id: 'consultas',  icon: '✦', label: 'Consulta',      cor: L.purple, bg: L.purpleBg },
    { id: 'financeiro', icon: '◬', label: 'Lançamento',    cor: L.green,  bg: L.greenBg },
    { id: 'triagem',    icon: '♡', label: 'Triagem',       cor: L.red,    bg: L.redBg },
    { id: 'estoque',    icon: '▣', label: 'Estoque',       cor: L.orange, bg: L.orangeBg },
  ]
  return (
    <div style={{ background: L.bg, border: `1px solid ${L.line}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
      <div style={{ fontWeight: 600, fontSize: 14, color: L.t1, marginBottom: 14 }}>⚡ Acesso Rápido</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 10 }}>
        {acoes.map(a => (
          <button key={a.id} onClick={() => nav(a.id)} style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
            padding: '14px 8px', borderRadius: 12, border: `1px solid ${L.line}`,
            background: L.surface, cursor: 'pointer', transition: 'all 0.15s',
          }}
            onMouseEnter={e => { e.currentTarget.style.background = a.bg; e.currentTarget.style.borderColor = a.cor + '40' }}
            onMouseLeave={e => { e.currentTarget.style.background = L.surface; e.currentTarget.style.borderColor = L.line }}
          >
            <span style={{ fontSize: 20, color: a.cor }}>{a.icon}</span>
            <span style={{ fontSize: 11, color: L.t2, fontWeight: 500, textAlign: 'center', lineHeight: 1.2 }}>{a.label}</span>
          </button>
        ))}
      </div>
    </div>
...
export default function PageDashboard({ profile, isMaster, nav }) {
  if (isMaster) return <DashboardMaster profile={profile} />
  return <DashboardClinica profile={profile} nav={nav} />
}
